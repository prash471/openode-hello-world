...
class Task {
  constructor(
    connectionSettings,
    requestSettings,
    completeCallback // Setup properties and send out initial HTTP request
  ) {
    // Callback methods
    ...
    const requestOptions = url.parse(
      this.connectionSettings.svrUrl + requestSettings.urlPathAndQuery
    );
    ...
    if (requestOptions.protocol == 'http:') {
      request = http.request(requestOptions, this.receiveResponse);
    } else {
      request = https.request(requestOptions, this.receiveResponse);
    }

    request.on('error', this.error);
    if (requestSettings.uploadData) {
      request.write(requestSettings.uploadData);
    }
    request.end();
  }

  receiveResponse(
    response // Bind listeners to Abbyy response events
  ) {
  ...
  readResponseData(
    responseData // Read data from Abbyy response into response buffer
  ) {
  ...
  processParsedResponseXml(
    err,
    parsedObj // Process the XML Response from Abbyy server
  ) {
    ...
    if (err) {
      ...
      this.error(new Error('Null output from xml2js'));
      ...
      const taskInfo = parsedObj.response.task[0];
      const id = taskInfo.id.toString();
      if (id.indexOf('00000000') > -1) {
        this.error(new Error('Null id received'));
      } else {
        if (!this.id) {
          this.id = id;
        ...
        if (this.id != id) {
          this.error(new Error('Id collision'));
        } else {
          ...
          if (status == 'Queued' || status == 'InProgress') {
            ...
            this.error(new Error('Unrecognized task status received'));
          }
        }
      }
    }
  ...
  waitForTaskCompletion() {
    // Wait 5 seconds (Abbyy recommended time) then poll task status
    const waitTimeout = 5000;
    setTimeout(this.getTaskStatus, waitTimeout);
  }

  getTaskStatus() {
    // Send out HTTP request to poll task status
    const requestOptions = url.parse(
      `${this.connectionSettings.svrUrl}/getTaskStatus?taskId=${this.id}`
    );
    requestOptions.auth = `${this.connectionSettings.appId}:${this.connectionSettings.pass}`;
  ...
  downloadResults(
    resultsUrl // Send out HTTP request to download results
  ) {
    const requestOptions = url.parse(resultsUrl); // [TODO] Test if this is always https url anyway
    let request;
    if (requestOptions.protocol == 'http:') {
      request = http.request(requestOptions, this.receiveResults);
    } else {
      request = https.request(requestOptions, this.receiveResults);
    }
  ...
  processResults() {
    // Return response (results) directly to user
    this.results = this.responseBuffer;
    this.finish();
  }

  error(
    error // Log any errors associated with Task and return it to user
  ) {
    this.error = error;
    this.callback(error);
  }
  ...
  finish() {
    // Task complete! Return it to the user
...
class AbbyyClient {
  ...
  _checkArgs(args) {
    ...
    if (args.length == 2) {
      if (Buffer.isBuffer(args[0]) || typeof args[0] === 'string') {
        uploadData = args[0];
      } else {
        parameters = args[0];
      }
    ...
    return { parameters, uploadData, userCallback: args[args.length - 1] };
  }
    ...
    if (
      ...
      if (typeof uploadData === 'string') {
        if (fs.existsSync(uploadData)) {
          uploadBuffer = fs.readFileSync(uploadData);
        } else {
          userCallback(new Error('File does not exist.'));
        }
      } else {
  ...
  processImage(/* [parameters], [upload data], callback */) {
    const args = this._checkArgs(arguments);
    this._runApiMethod('processImage', args.parameters, args.uploadData, args.userCallback);
  }

  processReceipt(/* [parameters], [upload data], callback */) {
    const args = this._checkArgs(arguments);
    this._runApiMethod('processReceipt', args.parameters, args.uploadData, args.userCallback);
  }
